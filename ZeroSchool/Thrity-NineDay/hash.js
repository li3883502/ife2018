...
function getSeach(){
    var content=[];
    content[0]=location.hash.replace('N','华北');
    ...
    content[1]=location.hash.replace('P','手机');
...
elcpdSelect[0].addEventListener('change',b);
elcpdSelect[1].addEventListener('change',b);
elcpdSelect[2].addEventListener('change',b);
function a(){
    var str=checkedValue(regionSelect);
...
function b(){
    var astr=a();
    var str=checkedValue(elcpdSelect);
    str=str.replace('手机','P');
    str=str.replace('笔记本','T');
    str=str.replace('智能音箱','S');
    history.pushState(null,null,'#'+astr+str);
}
