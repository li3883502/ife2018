function drawSvg(){
  ...
  tableWarpper.addEventListener("mouseover",function(evt){
   ...
   // 
    if(target.nodeName.toLowerCase()==='td'){
         var tr=(target.nodeName.toLowerCase()==='td')?target.parentNode:target.parentNode.parentNode;
      for(let i=0,j=0;i<tr.children.length;i++){
        if(/\d+/.test(tr.children[i].innerHTML)){//表格中可转化为数字的值进行描点
          array[0][j]=/\d+/.exec(tr.children[i].innerHTML);
          j++;
        }
      }
    }
  ...
  tableWarpper.addEventListener("mouseout",function(){
